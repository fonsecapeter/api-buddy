...
def _more_than_one_method_selected(opts: RawOptions) -> bool:
    return sum(cast(bool, opts[method]) for method in HTTP_METHODS) > 1
...
def _validate_data(data: str) -> Any:
    try:
        ...
        raise APIBuddyException(
            ...
            message=(
                "Please use valid json for: " f"{Fore.MAGENTA}{data}{Style.RESET_ALL}"
            ),
        )
...
def validate_options(opts: RawOptions) -> Options:
    ...
    valid_opts["<endpoint>"] = _validate_endpoint(cast(str, valid_opts["<endpoint>"]))
